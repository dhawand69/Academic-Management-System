...
async function populateFacultyMarksDropdown() {
    ...
    const myClasses = classes.filter(c => c.faculty.trim().toLowerCase() === facultyName.trim().toLowerCase());
...
function updateAdminMarksSemesterOptions() {
    ...
    if (currentSem !== 'all' && parseInt(currentSem) >= startSem && parseInt(currentSem) <= endSem) {
...
async function populateAdminMarksClassDropdown() {
    ...
    const semesterFilter = document.getElementById("adminMarksSemesterFilter").value;
...
async function loadAdminMarksTable() {
    ...
    try {
        ...
        // Render
        tbody.innerHTML = "";
        document.getElementById("adminMarksCount").textContent = `(${displayData.length})`;

        if (displayData.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;">No records found matching filters.</td></tr>';
            return;
        }
        ...
        displayData.forEach(item => {
            ...
            tr.innerHTML = `
                ...
                <td>${m.assignment}</td>
                <td>${m.attendance}</td>
                <td style="font-weight:bold; color:var(--color-primary);">${m.total}</td>
            `;
            tbody.appendChild(tr);
        });
...
async function exportInternalMarks(format) {
    ...
    if (classId === "all" && !confirm("Export consolidated report for ALL classes?")) {
        return;
    }
    ...
    if (format === 'csv' || format === 'excel') {
        ...
        downloadCSV(csvContent, selectedClass ? `Marks_${selectedClass.code}.csv` : `Marks_Consolidated.csv`);
    } else if (format === 'json') {
        const jsonData = { meta: { stats: { highest, lowest, average } }, data: reportData };
